...
const ProjectCard = ({ img, categ, title, descr, tech, liveDemo, gitHub }) => {
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-2xl overflow-hidden transition-all duration-300 hover:border-violet-500 hover:-translate-y-2 hover:shadow-xl hover:shadow-violet-500/10 hover:cursor-pointer">
      ...
      <div className="p-6 flex flex-col">
        ...
        <div className="flex flex-wrap gap-2 mt-5">
          {tech.map((item, index) => (
            <p
              className="bg-slate-700 text-cyan-400 text-xs px-3 py-1.5 rounded-full"
              key={index}
            >
              {item}
            </p>
        ...
        <div className="flex gap-3 mt-6">
          <button
            className="bg-violet-600 hover:bg-violet-700 text-white border-none px-4 py-2 rounded-xl hover:cursor-pointer"
            onClick={() => window.open(liveDemo, "_blank")}
          >
            LiveDemo
          </button>
          <button
            className="border border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-slate-950 px-4 py-2 rounded-xl hover:cursor-pointer transition-all duration-200"
